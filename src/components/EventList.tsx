...
export const EventList = observer(() => {
			...
			pointFormatter: function () {
				...
				arrow =
					disease.count > disease.prev
						...
						? dash
						: arrowDown;
				return `<div class="ptlabel">${
					point.series.name
				}: <b>${parseFloat(point.percentage).toFixed(
					1
				)}%</b>${arrow}</div>`;
			},
		},
	...
	const [mortalityFilter, setMortalityFilter] = React.useState<string>(
		undefined
	);
	...
	const filterTheDiseases = () => {
		...
		prevDiseases = new GenderFilter().apply(
			{ ...prevDiseases },
			genderFilter
		);
		...
		if (!store.currentOrganisation && !!store.selectedOrgUnit) {
			...
			sortedDiseases = calculatePrevDiseaseCounts(
				sortedDiseases,
				prevDiseases
			);
		}
			...
			<div id="topdiseaseswrapper">
				...
				>
					...
					>
						<Select
							placeholder={
								activeLanguage.lang["Filter Deaths"] ??
								"Filter Deaths"
							}
						...
						<Select
							placeholder={
								activeLanguage.lang["Gender"] ?? "Gender"
							}
							onChange={(e) => {
								if (!!e) {
									setGenderFilter(e);
