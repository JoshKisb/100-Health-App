...
export const DistrictSearchPopup: React.FC<SearchType> = observer(
	({
		disabled = false,
		searchType,
		dictatedContent,
		...
		const [level6Orgs, setLevel6Orgs] = useState([]);

		const searchEntity = (text: string) => {
			// const find
			const searchTypeArr = Array.isArray(searchType)
				? searchType
				: [searchType];

			const itemToSearch = searchTypeArr.flatMap((type) => {
				let res =
					type === validSearchTypes.facility
						? allFacilities
						: type === validSearchTypes.region
						? allRegions
						: type === validSearchTypes.district && !!parentName
						? allRegions.find(
								(reg) => reg.displayName === parentName
						  ).children
						: type === validSearchTypes.district
						? allRegions.flatMap((reg) => reg.children)
						: type === validSearchTypes.subCounty &&
						  !!parentName
						? allDistricts.find(
								(reg) => reg.displayName === parentName
						  ).children
						: type === validSearchTypes.subCounty
						? allDistricts.flatMap((reg) => reg.children)
						: type === validSearchTypes.level6
						? level6Orgs
						: [];
			...
			console.log("---SearchType---", searchTypeArr);
			console.log("---SearchArea---", itemToSearch);
			console.log("---Search---", potentialResults);
		...
		const resetSubCountySearch = () => {
			const newArr =
				searchType === validSearchTypes.facility
					...
					? allRegions.find((reg) => reg.displayName === parentName)
							.children
					: searchType === validSearchTypes.subCounty && parentName
					? allDistricts.find((reg) => reg.displayName === parentName)
							.children
					: [];
		...
		const getData = async () => {
			const searchTypeArr = Array.isArray(searchType)
				? searchType
				: [searchType];
			searchTypeArr.forEach(async (type) => {
        		console.log("---LOADING FOR---", type)
				if (type === validSearchTypes.facility) {
				...
				} else if (type === validSearchTypes.level6) {
					const orgs = await store.getLevel6Orgs();
					const actualOrgs = orgs.organisationUnits;
					setLevel6Orgs(actualOrgs);
				} else {
					...
					console.log("districts", districtsArr)
