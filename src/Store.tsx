...
class Store {
      ...
      try {
        ...
        if (!!this.activeLanguage?.lang) {
          ...
          this.availableDataElements =
            programStage.programStageDataElements.map((de: any) => {
  ...
  @action fetchLocalStorageEvent = async () => {
    ...
    if (!!this.lsdata['nationality']) {
      const nId = this.nationalitySelect?.find(
        (n) => n.name === this.lsdata['nationality']
      )?.id;
  ...
  @action addEvent = async (form: any) => {
    ...
    if (this.editing && this.currentEvent) {
      ...
      event = {
        ...event,
        event: this.currentEventObj?.event ?? this.lsdata?.ZKBE8Xm9DJG,
      };
      createMutation = { ...createMutation, data: event };
    }
    try {
